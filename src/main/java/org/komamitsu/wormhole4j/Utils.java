...
final class Utils {
  ...
  static int compareAnchorKeys(String s1, String s2) {
    int len1 = s1.endsWith(WormholeForStringKey.SMALLEST_TOKEN) ? s1.length() - 1 : s1.length();
    int len2 = s2.endsWith(WormholeForStringKey.SMALLEST_TOKEN) ? s2.length() - 1 : s2.length();
